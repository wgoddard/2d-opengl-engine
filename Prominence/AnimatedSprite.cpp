...
namespace Prominence {
	...
	AnimatedSprite::AnimatedSprite(ResourceManager & rm, Renderer & renderer, Logger & logger, std::string xml_file) : Sprite(rm, renderer), m_Logger(logger)
	{
		...
		TiXmlElement * elem = root->FirstChildElement("texture");
		...
		//{
		//	std::cout << index << '\t' << textures[index] << '\n';
		//}

		elem = root->FirstChildElement("sequence");
		...
		Uint32 tex_x = atoi(frameElem->Attribute("Tex_X"));
		Uint32 tex_y = atoi(frameElem->Attribute("Tex_Y"));
		Uint32 width = atoi(frameElem->Attribute("Width"));
		Uint32 height = atoi(frameElem->Attribute("Height"));
		...
		//frame->quad.v[0].tx = 0;				frame->quad.v[0].ty = 0;
		//frame->quad.v[1].tx = 1;				frame->quad.v[1].ty = 0;
		//frame->quad.v[2].tx = 1;				frame->quad.v[2].ty = 1;
		//frame->quad.v[3].tx = 0;				frame->quad.v[3].ty = 1;

		seq->frames.push_back(frame);

		//rest of frames
		while (frameElem = frameElem->NextSiblingElement("frame"))
		//while (frameElem = elem)
		{
			frame = new Frame();
			frame->time = atof(frameElem->Attribute("Time"));
			//Tex_X="206" Tex_Y="549" Width="39" Height="54" Anchor_X="19.5" Anchor_Y="27" Texture="0" />
			tex_x = atoi(frameElem->Attribute("Tex_X"));
			tex_y = atoi(frameElem->Attribute("Tex_Y"));
			width = atoi(frameElem->Attribute("Width"));
			height = atoi(frameElem->Attribute("Height"));
			texindex = atoi(frameElem->Attribute("Texture"));
			texid = textures[texindex]->loaded_tex;
			...
			texW = textures[texindex]->width;
			texH = textures[texindex]->height;
		...
		//rest of sequences // rest of frames
